...
pd1 <- pd %>%
  ...
  dplyr::select(-Sum, -Frequency) %>%
  dplyr::group_by(Variant)
pd1 <- pd1 %>%
  tidyr::spread(Variant, Percent) %>%
  tidyr::replace_na(list(other = 0,
                         pretty = 0,
                         really = 0,
                         so = 0,
                         very = 0)) %>%
  tidyr::gather(Variant, Percent, other:very)
# save data
write.table(pd1, "datatables/pd1.txt", row.names= F, sep = "\t")
# inspect data
pd1
```
...
We now extract the lexical diversity scores and begin with extracting example values.

```{r ampause_02_14, echo=T, eval = T, message=FALSE, warning=FALSE}
#famps <- c(famps, "0")
ldampaus <- ampaus %>%
  dplyr::select(Variant, Adjective) %>%
  dplyr::group_by(Variant) %>%
...
ldscores <- ldampaus %>%
  ...
  dplyr::select(Variant, LexicalDiversity)
ldampaus
```

Next, we extract LD values across apparent time.
...
ldscores <- ldampaus %>%
  dplyr::mutate(LexicalDiversity =round(Adjectives/Frequency, 1)) %>%
  dplyr::select(-Adjectivetypes) %>%
  dplyr::filter(Variant != "0")
write.table(ldscores, "datatables/LD_results_wother.txt", row.names= F, sep = "\t")
ldscores
```
...
```{r ampause_02_30, echo=T, eval = T, message=FALSE, warning=FALSE}
ccla_ampaus_selection <- ccla_ampaus %>%
  dplyr::filter(Freq_Adj > 10) %>%
  dplyr::select(-Freq_Variant, -Freq_other, -x2, -expected)
ccla_ampaus_selection
```

```{r ampause_02_31, echo=T, eval = T, message=FALSE, warning=FALSE}
library(tidyr)
freqadj <- ccla_ampaus_selection %>%
  dplyr::mutate(remove = ifelse(phi > 0.05, "keep", 
                                ifelse(phi < -0.05, "keep", "remove"))) %>%
...
pd3 <- ccla_ampaus %>%
  ...
  tidyr::replace_na(list(bad = 0,
                         ...
                         good = 0,
                         nice = 0)) %>%
  tidyr::gather(Adjective, phi, bad:nice) %>%
...
ccla_ampaus <- rbind(ccla_really_aus, ccla_very_aus, 
                     ccla_so_aus, ccla_pretty_aus, ccla_other_aus)
ccla_ampaus <- ccla_ampaus %>%
  dplyr::arrange(Age, Variant, Adjective) %>%
...
```{r ampause_02_52, echo=T, eval = T, message=FALSE, warning=FALSE}
Table5 <- ccla_ampaus %>%
  dplyr::filter(p <.05)
write.table(Table5, "datatables/CCLA_results_wother.txt", row.names= F, sep = "\t")
```
